...
>>>>>>> .r338
	...
	public class VehiclesHistoricController
	{
		...
		{
			return (String(HistoricoPosicoesVeiculoVO(item).dataEHora).toUpperCase().search(MainModel.getInstance().historicoSelecionado.toUpperCase()) >= 0) || (String(HistoricoPosicoesVeiculoVO(item).gpsDescSis).toUpperCase().search(MainModel.getInstance().historicoSelecionado.toUpperCase()) >= 0);
		}
...
<<<<<<< .mine
		...
		{
			TrafegusWS.getIntance().procuraHistoricoPosicoes(procuraHistoricoPosicoesResultHandler, AdvancedSearcHistoricoVO(event.genericVO))
		}
		
		private function procuraHistoricoPosicoesResultHandler(event:ResultEvent):void {
			MainModel.getInstance().totalHistoricoPosicoes = ParserResult.parserDefault(event).length;
			this.solicitaHistoricoPosicoesHandler(event);
		}
=======
